...
const findFavouritesTrips = async(id) => {
    ...
    const sql = `SELECT db_viajes2.trips.*, COUNT(votes.id) AS votes FROM trips left join favourites on favourites.IdTrip = trips.id left join votes on votes.IdTrip = trips.id where favourites.IdUser = ? GROUP BY trips.id ORDER BY votes DESC;`;
  ...
  const findExist = async (id, idTrip) => {
    const pool = await getConnection();
    const sql = `SELECT * FROM db_viajes2.trips left join favourites on favourites.IdTrip = trips.id where favourites.IdUser = ? AND favourites.IdTrip = ?`;
